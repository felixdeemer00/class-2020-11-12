...
mod_2_wfl <- workflow() %>%
  add_model(linear_reg() %>%
            set_engine("stan")) %>%
  add_recipe(recipe(reg_chg ~ treatment + poverty_n + county,
                    data = week_10_train))

predictions_2 <- mod_2_wfl %>%
  fit(data = week_10_train) %>%
  predict(new_data = week_10_test)
...
If the model is fitted to a certain dataset, and is tested on that same exact dataset, it will correspond well 
to that specific dataset, but it may work poorly on other data as they were not used to create the model, which
is overfitted to the initial dataset.
...
new_data <- tibble(treatment = "local", 
                   poverty_n)
